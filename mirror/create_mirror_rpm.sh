...
yum --installroot $CHROOT install -y $RPM_EPEL || true
yum --installroot $CHROOT install -y yum-utils
yum-config-manager --installroot $CHROOT --enable $RPM_EXTRAS $RPM_OPTIONAL
yum-config-manager --installroot $CHROOT --add-repo $MY_SQL_REPO
yum-config-manager --installroot $CHROOT --add-repo $CLOUDERA_MANAGER_REPO
yum-config-manager --installroot $CHROOT --add-repo $SALT_REPO
yum-config-manager --installroot $CHROOT --add-repo $AMBARI_REPO

cp /etc/yum.repos.d/* $CHROOT/etc/yum.repos.d/
curl -LJ -o $CHROOT/etc/yum.repos.d/ambari-legacy.repo $AMBARI_LEGACY_REPO
...
curl -LOJf https://archive.fedoraproject.org/pub/epel/RPM-GPG-KEY-EPEL-7

# import repo keys
rpm --root $CHROOT --import *
...
(yumdownloader --installroot $CHROOT --resolve --archlist=x86_64 --destdir $RPM_REPO_DIR $RPM_PACKAGE_LIST 2>&1) | tee -a yum-downloader.log; cmd_result=${PIPESTATUS[0]} && if [ ${cmd_result} != '0' ]; then exit ${cmd_result}; fi
